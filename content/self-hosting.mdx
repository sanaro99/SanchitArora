---
title: "My Self-Hosting Journey: From Google Photos to a Cross-Platform Private Cloud"
publishedAt: "2025-05-13"
summary: "TrueNAS home server using old laptop — Discover how I repurposed an old laptop into a feature-rich NAS with ZFS, Immich, backups, and a Windows VM, all under 500 USD"
---

![My TrueNAS Setup](/truenas-setup.jpg)
...
![Disk Configuration](Screenshot_2025-05-13_225455.png)
...
* **Future Backup Plan**: I plan to sync data to a friend's remote TrueNAS using **ZFS replication**, creating offsite protection.

**Image: Network bonding configuration**
![Network Interfaces](Screenshot_2025-05-13_230256.png)
...
* **SMB (Samba) Shares** are ACL-isolated: each family member has a private space (e.g., `/users/rachit-arora`), with a separate **shared folder** (`/common`) for movies and files.
* **Audit logging** is enabled across shares.
* No one sees what they're not supposed to.

**Image: SMB Shares configuration**
![SMB Shares](Screenshot_2025-05-13_230522.png)

---

## Functionalities: More Than Just Storage

This isn’t just a file dump—it’s a smart, extensible home cloud:
...
Using **FolderSync**, our family phones back up photos and files **daily** to their respective SMB shares.

### 3. **Personal Google Photos Alternative**

I migrated all albums from Google using `immich-go`. Now we use **Immich**, a beautiful self-hosted media server:
...
![Immich Usage](Screenshot_2025-05-13_225635.png)

### 4. **Windows Virtual Machine**

A lightweight Windows VM (`SanchitCloud`) runs on TrueNAS:

* 6 GB RAM, 2 cores
* RDP access
* Accessible even on phone

**Image: Virtual machine details**
![VM Configuration](Screenshot_2025-05-13_230841.png)
...
![Apps Running](Screenshot_2025-05-13_230544.png)
...
![System Monitoring](Screenshot_2025-05-13_225711.png)

---

## Graceful Shutdowns

While my UPS doesn’t have network monitoring, it gives a 15-minute buffer. I plan to use the **laptop’s mic** to detect the UPS beep and trigger an automatic shutdown.
...
![Datasets](Screenshot_2025-05-14_004739.png)
